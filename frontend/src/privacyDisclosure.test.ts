...
describe('privacy disclosure', () => {
  test('states the local-only and stateless product contract', () => {
    ...
    expect(appSource).toContain('Crackle Date does not send or keep user data on our servers. Puzzle, equation, and validation requests are processed only long enough to respond.');
    ...
    expect(appSource).toContain('Crackle Date is free to play. There are no ads, paid version, purchases, accounts, tracking, public profiles, or cloud gameplay history.');
  ...
  test('explains that missing browser history cannot be restored', () => {
    const appSource = readFileSync(new URL('./main.tsx', import.meta.url), 'utf8');
    expect(appSource).toContain('Clearing browser data, using private browsing, switching browsers, or changing devices can remove local history. Crackle Date does not keep, receive, or restore a server copy.');
  });
});
